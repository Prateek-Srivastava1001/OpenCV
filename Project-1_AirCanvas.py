...
colorIndex = 0
myPoints = [] # LIST of x,y, color index
...
#DRAWING
def drawOnCanvas(myPoints, colorIndex): #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>TO BE COMPLETED>>>>>>>>>>>>>>>>>>>>>>
    # LINKING WITH UI
    if x in range(5, 155) & y in range(5, 101):
        colorIndex = 3
    for point in myPoints:
        ...
        if colorIndex == 3:
           colorIndex = 0 # DELETE LIST>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>TILL HERE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
